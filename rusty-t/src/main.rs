...
fn decode_response(bytes: Vec<u8>) -> Result<UdpResponse, std::io::Error> {
    ...
    match msg_type {
	    ...
	    let _metadata_length = reader.read_u16::<BigEndian>()?;

            let mut metadata_json: Vec<u8> = vec![];
	    reader.read_to_end(&mut metadata_json)?;

            return Ok(UdpResponse::UdpPong {
                pong_payload: Option::Some(PongPayload {
                    series_id,
                    frame_count,
		    metadata: String::from_utf8_lossy(&metadata_json[..]).to_string(),
                }),
            });
        }
        3 => {
                        ...
                        UdpResponse::UdpPong { pong_payload } => match pong_payload {
                            Some(payload) => {
                                if Some(payload.series_id) == prior_series_id {
                                    ...
                                    info!("no series: new series (metadata {0}), switching state, waiting", payload.metadata);
